...
- **macOS Keychain backend (`keychain://`)** — the zero-dependency option:
  ...
  Detected last, so an installed vault CLI stays the suggested default; on
  non-macOS it errors clearly at resolve time and `mcp-doctor` warns rather
  than failing. The vault backends (`op`/`sops`/`bw`) remain the
  recommendation for teams and multi-machine setups: the Keychain has no sync
  or sharing story. Refs are recognized as safe by both guards, `mcp-doctor`,
